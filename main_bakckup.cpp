...
#define			alloying_elements					/*"C", "Si"*/ "Mn" /*, "Mo",*/ /*"Ni"*/				// iloúÊ pierwiastkÛw 2/4
...
const int		no_of_cells_x						= 100,											// [#] // 2021-03-23 -> 100
				...
				dt_multiplier = 1.0046,										// [dimensionless]
				initial_dt = /*5.0e-3*/  5.0e-2;				//5.0e-6;	/*0.00016285044;*/
...
const double	density											= C_cell::density,
				...
				mobility[no_of_elements]						= { 1.38338e-18, 3.10748e-19 };											// [s * mol / kg]
...
double			mobility_coefficient[no_of_phases + 1][no_of_elements],											// phases -> 
				...
				temperature = 1170 + 273.15,										// [K]
				
				initial_conc[no_of_phases][no_of_elements - 1] = { 0.0, /*2.77,*/ 0.0/*, 2.77*/},	// iloúÊ pierwiastkÛw 4/4	// [wt. %, ph0_el0, ph0_el1, ph1_el0, ph1_el1]			
				...
				initial_conc2[no_of_phases][no_of_elements - 1] = { 7.0/*, 0.05*/,/*0.05, 0.5, 0.4*/ /*||*/ 7.0/*, 0.05*/ /*, 0.05, 0.7, 0.6*/ };	// iloúÊ pierwiastkÛw 4/4	// [wt. %, ph0_el0, ph0_el1, ph1_el0, ph1_el1]			
...
const int		color_depth = 1024,
				...
				//no_of_screen_loop_stepps = 1,				// iloúÊ razy ile program daje znac, øe nadal liczy; ca≥kowita iloúÊ krokÛw (iloczyn trzech loop_stepps)
				//no_of_file_loop_stepps = 100,					// (iloúÊ plikÛw = file * screen)
				...
				//no_of_calculations_loop_stepps = 10000;	// interwa≥ pomiÍdzy wypisaniami do pliku 100000
...
void initial_temperature()
{
	for (int j = 0; j < no_of_cells_y; j++)
	{
		for (int i = 0; i < (no_of_cells_x); i++)
		{
			...
			tab_temperature[j][i] = temperature/* + i*/;
		}
	}
}
//*********************************
...
void initial_concentration()
{
	for (int k = 0; k < no_of_cells_y; k++)
	{
		...
		for (int j = 50; j < (no_of_cells_x); j++)
		{
			for (int i = 0; i < no_of_elements - 1; i++)
			{
				if (tab_initial_state[k][j] == C_cell::list_of_states::FCC)
