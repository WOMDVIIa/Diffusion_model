...
#define			alloying_elements					"Al", "Ni" /*"Cu"*/ /*"C", "Si"*/ /*"Bi"*/ /*, "Mo",*/ /*"Ni"*/				// iloúÊ pierwiastkÛw 2/4
char			*element[no_of_elements]			= {alloying_elements, main_element};		// pierwiastek bazowy na koÒcu listy
...
const double	density											= C_cell::density,
				//mobility[no_of_elements]						= {6.01362E-18, 6.01362E-18, 1.87e-19};		// Fe-Al-Ni pierwsza prÛba
				//mobility[no_of_elements]						= { 6.01362E-18, 1.6E-19, 1.87e-19 };		// Fe-Al-Ni druga prÛba
				mobility[no_of_elements]						= { 6.01362E-18, 1.6E-19, 1.87e-19 };		// Fe-Al-Ni trzecia prÛba
				//mobility[no_of_elements]						= { 3.585E-13, 3.585E-13 };						//e-12,e-15,e-12// [kg * mol / (J * s * m)] jakoú to trzeba rozbiÊ -------- to dzia≥a 3e-17
				//mobility[no_of_elements]						= { 1.85415E-13, 1.85415E-13};						//e-12,e-15,e-12// [kg * mol / (J * s * m)] jakoú to trzeba rozbiÊ -------- to dzia≥a 3e-17
				//mobility[no_of_elements]						= { 1e-10, 1e-12, 1e-16 };						//e-12,e-15,e-12// [kg * mol / (J * s * m)] jakoú to trzeba rozbiÊ		// 2021-05-13:	= {1e-18, 1e-18, 1e-18};		tyle powinno byÊ = {1e-10, 1e-12, 1e-16};
//tu bylo				//mobility[no_of_elements]						= { 6.01362E-18, 3.00681E-14};											// [s * mol / kg]
				//mobility[no_of_elements]						= { 4.64e-15, 1.94e-17, 6.83e-20 };			// do pierwszych dobrych potrÛjnych!								// [s * mol / kg]
...
double			mobility_coefficient[no_of_phases + 1][no_of_elements],											// phases -> 
				...
				// by≥o poniøej
				//initial_conc[no_of_phases][no_of_elements - 1]	= { 0.8, /*, 0.5, 0.4*/ /*||*/ 0.8/*, 0.7, 0.6*/ };	// iloúÊ pierwiastkÛw 4/4	// [wt. %, ph0_el0, ph0_el1, ph1_el0, ph1_el1]			
				//initial_conc3[no_of_phases][no_of_elements - 1] = { 29.359, /*69.0,*/ /*0.05, 0.5, 0.4*/ /*||*/ 29.359/*, 69.0*//*, 0.05*/ /*, 0.05, 0.7, 0.6*/ };	// iloúÊ pierwiastkÛw 4/4	// [wt. %, ph0_el0, ph0_el1, ph1_el0, ph1_el1]			


				//initial_conc[no_of_phases][no_of_elements - 1]	= {0.615, 3.0/*, 0.5, 0.4*/,  /*||*/ 0.615, 3.0/*, 0.7, 0.6*/};	// iloúÊ pierwiastkÛw 4/4	// [wt. %, ph0_el0, ph0_el1, ph1_el0, ph1_el1]
...
const int		color_depth = 1024,
				no_of_screen_loop_stepps = 60,				// iloúÊ razy ile program daje znac, øe nadal liczy; ca≥kowita iloúÊ krokÛw (iloczyn trzech loop_stepps)
				no_of_file_loop_stepps = 5,					// (iloúÊ plikÛw = file * screen)
				no_of_dt_loop_stepps = 5,					// tyle razy zostanie wykonana pÍtla calculations zamin zostanie zapisany plik (przy zmiennym dt tyle razy zmieni siÍ zanim plik zostanie zapisany)
				no_of_calculations_loop_stepps = 50000;	// interwa≥ pomiÍdzy wypisaniami do pliku 100000
				//no_of_screen_loop_stepps = 10,				// iloúÊ razy ile program daje znac, øe nadal liczy; ca≥kowita iloúÊ krokÛw (iloczyn trzech loop_stepps)
				//no_of_file_loop_stepps = 10,					// (iloúÊ plikÛw = file * screen)
				//no_of_dt_loop_stepps = 1,					// tyle razy zostanie wykonana pÍtla calculations zamin zostanie zapisany plik (przy zmiennym dt tyle razy zmieni siÍ zanim plik zostanie zapisany)
				//no_of_calculations_loop_stepps = 100;	// interwa≥ pomiÍdzy wypisaniami do pliku 100000
...
double calculate_mobility(C_cell *cell, int *phase, int *element)
{
	...
	////dla Fe-Ni - pierwotne, s≥abe wyznaczenie ale dzia≥ajπce 
	//if (*element == 0) // Ni
	//{
	...
	//	//return 2.36281E-23 * pow(temp_wtpctNi, 3) - 1.19255E-21 * pow(temp_wtpctNi, 2) + 2.47366E-20 * temp_wtpctNi - 6.46946E-22; // versja dopasowana do wynikÛw dictry przy DFe const
	...
	//	return -3.76158E-37 * pow(temp_wtpctNi, 3) + 1.05685E-22 * pow(temp_wtpctNi, 2) + 2.50758E-21 * temp_wtpctNi + 8.14994E-20; // versja z danych Smithels przy DFe var v3 - mnoønik 1.2
	//}
	//else // Fe
	//{
	...
	//	return -3.76158E-37 * pow(temp_wt_perc, 3) + 1.05685E-22 * pow(temp_wt_perc, 2) + 2.50758E-21 * temp_wt_perc + 8.14994E-20; // DFe var v3 - mnoønik 1.2
	//}
	...
	//		return (A_of_T + B_of_T * temp_wt_perc) / 10000 / (gas_constant * cell->temperature); // dla zaleønoúci od cC
	//		//return (A_of_T + B_of_T * 0.75) / 10000 / (gas_constant * cell->temperature);	// niezaleøne od cC
	//	}
	//}
	//else // Fe
	//{
	...
	//		//return 3.72639E-15; // +20% do mniejszego v3 (wczeúniej +25%)
...
void initial_concentration()
{
	for (int k = 0; k < no_of_cells_y; k++)
	{
		...
		for (int j = 0; j < 50; j++)	// dla 2 materia≥Ûw
		//for (int j = 0; j < 58; j++)		// dla 3 materia≥Ûw
		{
		...
		for (int j = 50; j < (no_of_cells_x); j++)	// dla 2 materia≥Ûw
		//for (int j = 58; j < 62; j++)	// dla 3 materia≥Ûw
		{
			for (int i = 0; i < no_of_elements - 1; i++)
			{
				if (tab_initial_state[k][j] == C_cell::list_of_states::FCC)
		...
		// úrednia przy nieparzystej iloúci komÛrek
		//for (int i = 0; i < no_of_elements - 1; i++)
		//{
...
void initialization()
{
	...
	set_calculatoins_gap();															// dodane øeby liczyÊ D,B (c_Fe, c_Ni)
	calculate_phase_fraction_and_masses();											// dodane øeby liczyÊ D,B (c_Fe, c_Ni)
	calculate_chemical_potentials_and_concentrations_in_entire_cell_table();		// dodane øeby liczyÊ D,B (c_Fe, c_Ni)
	
	//dla var D
	//calculate_mobility_coefficients_table();										// dodane øeby liczyÊ D,B (c_Fe, c_Ni)
...
void calculate_delta_mass(C_cell *current_cell, C_cell *next_cell, int *y, int *x, int *phase, int *dimension)
{
	for (int element = 0; element < no_of_elements; element++)
	{
		...
		//// 2021-04-29 dla zaleønoúci D od T i c
		...
		//// 2021-09-13 zmiana strumienia w zaleønoúci czy jest wychodzπcy czy przychodzπcy z komÛrki o masie wiÍkszej lub mniejszej niø nominalna
		//// delta m_________________________________________________________________________________________________ = (- B * dx * dt / 2)__________ 
		...
		//}
		//// do dupy to, zrobiÊ dwie sk≥adowe, jedna jak by≥a z naprÍøeniami majπcymi wp≥yw na potencja≥ chemiczny, druga relaksacja naprÍøeÒ - samoistne przechodzenie masy z rejonÛw o wiÍkszej masie do mniejszej
	}
}
//*********************************
...
// log:
// 2021-05-06: problem z nieustalaniem rÛwnej masy, rozwiπzanie: zwiÍkszyÊ czu≥oúÊ na odchylenie od rÛwnowagi przy mu_coef: zwiÍkszyÊ mnoønik ALE to powoduje powstawanie fal na masie komÛrki,
// albo zmniejszyÊ krok albo inaczej rozwiπzaÊ problem
// 2021-05-06: podjÍto rozwiπzanie ze zmniejszeniem ktoku czaswego - sukces, moøna wyliczyÊ obszar i czas obliczeÒ!
// 2021-05-13: pierwsze obliczenia czasu: 5 min obliczeÒ 2.4e-5 sec czasu rzeczywistego 
// dx-7 dt-6
// 2021-05-13: zrobiono rzeczywiste mobilnoúci, powalczyÊ z czasem obliczeÒ (zmienne dt)
// 2021-05-14: dt = 5e-6 wyglπda, øe dzia≥a
// 2021-05-19 wyniki ze sta≥ym dt=1e-5 i ze zmiennym nie pokrywajπ siÍ, przetestujÍ wyniki dla sta≥ego dt 1e-5 i 5e-6 czy sπ identyczne, trzeba znaleüÊ maksymalne dt, ktÛre daje dobre wyniki 
// po sprawdzeniu dt = 1e-5, 5e-6, 2.5e-6 : wyniki bardzo do siebie zbliøone, przy sta≥ym 1e-5 problemu nie ma ale moøe on byÊ szybko zauwaøalny dla niewiele wzrastajπcego dt juø w pierwszych krokach
// dlatego zaczπÊ nie od 1e-5 a od 5e-6
// q=1,0046 jest wystarczajπce po porÛwnaniu z q=1,0025
// 2021-05-19 wznawianie obliczeÒ dla var dt dzia≥ bez zarzutu
// 2021-05-24 dodano kolejnπ pentlÍ redukujπcπ iloúÊ plikÛw a nie sprawiajπcπ, øe rzadziej dochodzi do zwiÍkszenia dt